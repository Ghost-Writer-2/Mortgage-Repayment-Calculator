(function(){
  ...
  const NUMBER_INPUTS = Array.from(document.querySelectorAll('.js-number-input'));
...
//  removes default html validation information
  ...
  function showError() {
    ...
    NUMBER_INPUTS.forEach((input) => {
      ...
      if (input.validity.rangeOverflow || input.validity.rangeUnderflow) {
        error(`Please make sure the value is within ${input.min} - ${input.max}`)
      }
    })
  ...
  var calculate = {
    ...
    totalPayment: function() {
      return parseFloat((calculate.repayment() * (NUMBER_INPUTS[1].value * 12)) .toFixed(2));
    }
  }
